...
const getRequestForOwner = async (userId) => {
  ...
  for (let i = 0; i < owner.rooms.length; i++) {
    rooms.push(mongoose.Types.ObjectId(owner.rooms[i].roomId));
  }
...
const getRequestOfUser = async (userId) => {
  return await Request.find({ userId: userId }).populate(
    "userId building roomId"
  );
};
...
const approveRequest = async (requestId, userId) => {
  ...
  if (request.notificationType == NotificationType.owner) {
    ...
    if (request.building) {
      ...
      let building = await addNewOwnerToBuilding(
        newOwner._id,
        request.building,
        userId
      );
