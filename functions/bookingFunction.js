...
const addBooking=async (userId,date,type,bookingText)=>{
    ...
    let booking = new Booking({
        ...
        type,
        date,
        building:tenant.roomId.building,userId,
        booking:bookingText
    });
...
const getBookingsForOwner = async(userId)=>{
    ...
    return await Booking.find({building:{$in: buildings}}).populate('userId building');
}
...
return await Booking.findByIdAndUpdate(bookingId,{status:requestStatus.accept});
}
const rejectBooking = async(bookingId)=>{
    return await Booking.findByIdAndUpdate(bookingId,{status:requestStatus.reject});
    }
module.exports = {acceptBooking,rejectBooking,addBooking,getBookingsForOwner,getBookingsForUser};