...
const addBuilding=async(name,address,userId,rooms)=>{
    let owner = (await Owner.findOne({userId}).select('_id buildings'));
    if(!owner&&owner!==null)
    {
...
const getAllBuilding=async()=>{
    return await Building.find().populate('agentId ownersId tenantId rooms.roomId')
    .populate({
        path: "agentId",
        populate: {
          path: "userId"
...
const getAllBuildingOfOwner=async(id)=>{
    return await Building.find({ownersId:id}).populate('agentId ownersId tenantId rooms.roomId')
    .populate({
        path: "agentId",
        populate: {
          path: "userId"
...
const addAgentToBuilding=async(agentId,buildingId)=>{
    ...
    building = await building.save();
    return await Building.findById(building._id).populate('agentId ownersId')
    .populate({
        path: "agentId",
        populate: {
          path: "userId"
...
const addNewOwnerToBuilding=async(ownerId,buildingId,userId)=>{
    ...
    //data saving
    previousOwner= await Owner.updateOne({_id:previousOwner._id},{
        $pull:{
...
const updateBuilding=async(buildingId,userId,name,address,rooms)=>{
  ...
  if(building.ownersId.toString()!=owner._id.toString()){
    return{error:'unauthorized',message:'you are not allowed to edit building'};
  }
  building.name=name;
