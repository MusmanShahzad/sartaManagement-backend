...
module.exports = {
  ...
  Mutation: {
    ...
    RejectRequest: async (parent, args, context) => {
      let Errors = [];
      let Data = await rejectRequest(args.requestId, context.userId);
      if (Data.error) {
        Errors.push(Data);
        return {
          Errors,
    ...
    AddChatOwner: async (parent, args, context) => {
      ...
      if (args.file) {
        try {
          ...
          let Data = await addChatOwner(
            args.buildingId,
            args.message,
            server + "uploads/" + _filename,
            context.userId
          );
          if (Data.error) {
            Errors.push(Data);
  ...
  Subscription: {
    ChatOwnerUpdate: {
      subscribe: withFilter(
        () => pubsub.asyncIterator("ChatOwnerUpdate"),
        (payload, variables) => {
